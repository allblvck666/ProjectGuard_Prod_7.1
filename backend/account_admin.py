...
def _active(user):
    return user and user.get('is_active', 1) not in (0, '0', False)
...
def change_account(actor_id, target_id, data=None, *, delete=False, hard_delete=False):
    ...
    with _authorized_change(actor_id) as (conn, cur, actor):
        ...
        if target['role'] == 'superadmin' and _active(target) and removes_owner:
            cur.execute("SELECT COUNT(*) AS count FROM users WHERE role='superadmin' AND COALESCE(is_active,1)<>0")
...
def create_account(actor_id, data):
    ...
    with _authorized_change(actor_id) as (conn, cur, actor):
        ...
        cur.execute(db._adapt_query('INSERT INTO users(tg_id,tg_username,first_name,role,created_at) VALUES(?,?,?,?,?)'),
